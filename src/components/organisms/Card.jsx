...
export const Card = ({ title, image = null, body, stack, redirect = false, onClick, variant = "default", ...props }) => {
		...
		case "neo":
			return (
				<motion.div onClick={onClick}
					...
					className={"item flex gap-2 flex-col w-full md:w-[260px] hover:scale-105 transition p-4 items-center cursor-pointer gap-2 " + (redirect ? "justify-around" : "justify-between")}
				>

					<div className="flex flex-col gap-2">
						{loading && image && <div className="image-placeholder placeholder" />}
						{image && <img src={image} id="image" onLoad={() => setLoading(false)} alt="logo" className={`aspect-[16/11] md:w-[250px] rounded-md object-fill ` + (loading ? "hidden" : "flex")} />}
						<div className="flex flex-row gap-2 items-center justify-center">
							<h2 className="md:text-lg text-2xl font-[500]">
								{title}
							</h2>
							{props.fav && <div className="px-2 py-1 bg-yellow-500 dark:bg-yellow-600 rounded-full text-[#f1f1f1] font-bold text-[9px]">{t('fav')}</div>}
						</div>
						<p className="font-[400] text-sm md:text-md opacity-70">
							{body}
						</p>
					</div>
